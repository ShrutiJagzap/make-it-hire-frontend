...
const VideoInterview = ({ open, onClose, resumeData, sessionId, userId }) => {
  ...
  useEffect(() => {
    const checkAIService = async () => {
      try {
        ...
        const response = await fetch(`${API_CONFIG.ai}/health`, {
          method: 'GET',
        ...
        if (response.ok) {
          ...
          console.warn('⚠️ AI Service returned non-200 status');
          setIsAIServiceAvailable(false);
        }
      } catch (error) {
        console.warn('⚠️ AI Service is not available:', error.message);
        setIsAIServiceAvailable(false);
  ...
  // Start identity verification - FIXED with timeout and better error handling
  const startVerification = async () => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (!isAIServiceAvailable) {
        ...
        try {
          ...
          const healthResponse = await fetch(`${API_CONFIG.ai}/health`, {
            method: 'GET',
            signal: controller.signal
          });
          ...
          setError('AI Service is not available. Please ensure it is deployed and running.');
          setLoading(false);
          return;
        }
      }
      ...
      console.log("User ID:", userId_local);
      console.log("AI Service URL:", API_CONFIG.ai);
      ...
      try {
        ...
        if (!response.ok) {
          const errorText = await response.text();
          console.error("Verification HTTP error:", response.status, errorText);
          
          if (response.status === 502) {
            throw new Error("AI service is waking up. Please wait and try again.");
          } else if (response.status === 503) {
            throw new Error("AI service is temporarily unavailable. Please try again.");
          } else if (response.status === 404) {
            throw new Error("AI service endpoint not found. Please contact support.");
          } else {
            throw new Error(`Verification failed (${response.status}): ${errorText || 'Unknown error'}`);
          }
        }
        ...
        if (fetchError.name === 'AbortError') {
          setError("Request timed out. The AI service might be starting up. Please try again.");
        } else if (fetchError.message.includes('Failed to fetch')) {
          setError("Cannot connect to AI service. Please ensure it's deployed and running on Render.");
        } else {
          setError(`Verification failed: ${fetchError.message}`);
        }
      }
      ...
      <DialogContent sx={{ p: 3 }}>
        ...
        {!isAIServiceAvailable && (
          <Alert severity="warning" sx={{ mb: 3 }}>
            AI Service is currently unavailable. Please check the service status and try again.
            <Button 
              size="small" 
              sx={{ ml: 2 }}
        ...
        {step === 'verification' && (
          <Box sx={{ textAlign: 'center', py: 4 }}>
            ...
            <Button 
              ...
              disabled={loading || !isAIServiceAvailable} 
              sx={{ px: 4 }}
            >
              {loading ? <CircularProgress size={24} /> : 'Start Verification'}
            </Button>
        ...
        {step === 'questions' && questions.length > 0 && (
          <Box>
            <Paper sx={{ p: 4, mb: 3, bgcolor: '#f5f3ff', position: 'relative' }}>
              <Typography variant="h6" sx={{ fontWeight: 'medium' }}>{questions[currentQuestion]}</Typography>
              <IconButton 
                size="small" 
                sx={{ position: 'absolute', top: 8, right: 8 }}
